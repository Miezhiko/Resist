...
auto simple_match( const algebraic_data_type< TR ... > & adt, const T & t ) {
  ...
  typename adt_type::template match_visitor<T, ret_type> smv {
    t
  };
...
template<>
struct pattern_tester<multi_tester<>> {
  static bool match_pattern( const std::tuple<> & ) {
    return true;
  }
};
...
struct pattern_tester<constructor_indicator< self_type, which, PR ... >> {
  struct tester_helper {
    ...
    bool operator ( )( const L &, const R & r ) const {
      ...
      return make_expansion(
        ...
        [](const std::false_type &, const auto & ) {
          return false;
        })
  ...
  static bool match_pattern( const algebraic_data_type<ARG ...> & s ) {
    return simple_match( s, tester_helper( ) );
  }

  template<typename T>
  static bool match_pattern( const std::tuple<T> & t ) {
    return match_pattern( extract_recursive_wrapper( std::get<0>( t ) ) );
  }
...
struct pattern_tester<any_tester<FIRST, REST ...>> {
  ...
  static bool match_pattern( const T & t ) {
    return pattern_tester<FIRST>::match_pattern( t )
        || pattern_tester<any_tester<REST ...>>::match_pattern( t );
  }
};
